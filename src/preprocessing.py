...
def preprocess(x: tf.Tensor, y: tf.Tensor, n_labels: int, augment: bool = True):
    x = tf.cast(tf.reshape(x, (-1,)), tf.float32) / 125.5 - 1.
    y = tf.cast(y, tf.float32)

    if augment:
        # generate negative sample randomly
        if .5 < tf.random.uniform([]):
...
@gin.configurable
def get_datasets(name: str = 'mnist', batch_size: int = 512, valid_percentage: int = 20, n_labels: int = 10):
    train_ds, valid_ds, test_ds = tfds.load(
        name, split=[
            f'train[:{int(100 - valid_percentage)}%]', f'train[{int(100 - valid_percentage)}%:]', 'test'],
        as_supervised=True
    )
    train_ds = train_ds.map(lambda x, y: preprocess(x, tf.cast(y, tf.float32), n_labels)).\
